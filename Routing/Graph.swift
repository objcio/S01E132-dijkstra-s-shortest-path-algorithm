...
extension Graph {
    ...
    func shortestPath(from: Coordinate, to: Coordinate) -> [Coordinate]? {
        ...
        var seen: Set<Coordinate> = []
        var distances: [Coordinate:(distance: CLLocationDistance, previous: Coordinate)] = [:]
        var queue: [(coordinate: Coordinate, distance: CLLocationDistance)] = [(coordinate: from, distance: 0)]
        while let (coordinate, dist) = queue.popLast() {
            guard coordinate != to else { break }
            guard !seen.contains(coordinate) else { continue }
            seen.insert(coordinate)
            for dest in edges[coordinate] ?? [] where !seen.contains(dest.coordinate) {
                let newDist = dist + dest.distance
                if let (existingDist, _) = distances[dest.coordinate], existingDist < newDist {
                    continue
                }
                distances[dest.coordinate] = (distance: newDist, previous: coordinate)
                queue.append((coordinate: dest.coordinate, distance: newDist))
            }
            queue.sort { $0.distance > $1.distance }
        }
        guard var current = distances[to]?.previous else {
            return nil
        }
        var result: [Coordinate] = [to]
        while current != from {
